...
def find_repo_root(cwd: Optional[Path] = None) -> Path:
    """
    Find the git repository root (backward compatibility alias).
    ...
    """
    resolver = RepoResolver(cwd)
    return resolver.resolve_repo_root()


# Alias for backward compatibility
def setup_gimi(cwd: Optional[Path] = None) -> Tuple[Path, Path]:
    """
    Setup gimi repository structure (backward compatibility alias).

    Args:
        cwd: Current working directory. If None, uses Path.cwd().

    Returns:
        Tuple of (repo_root, gimi_path).
    """
    return initialize_repo(cwd)


# GimiPaths alias for backward compatibility
class GimiPaths(RepoResolver):
    """Backward compatibility alias for RepoResolver."""
    pass
...
class NotAGitRepoError(RepoError):
    """Error raised when not in a git repository."""
    pass
